...
int& List::operator[](const int index)		//мы не можем просто вернуть данные из списка, для этого сначала необходимо перейти в нужную ячейку
{
	...
	{
		throw std::exception("Выход за границы списка.");	//если индекс выходит за список, то выбрасывается исключение
	}
...
{
	if (iter.head == nullptr) { throw std::exception("Итератор не указывает на элемент"); }	//проходит проверка указывает ли итератор, на какой-либо элемент списка
	if (iter.current == nullptr) { throw std::exception("Итератор не указывает на элемент"); }	//если итератор никуда не указывает, то выбрасывается исключение
	Node* temp = iter.head;
...
List List::operator*(const List& other)
{
	...
	{
		throw std::exception("Размеры списков разные");
	}
	List temp;
	Node* l1 = this->head;
	Node* l2 = other.head;
