...
vector<forward_list<Data>> CreateHashTable(const vector<Data>& arr)       //функция генерирущая хешированую таблицу
{
    colision = 0;
    int fl, size = arr.size();
    vector<forward_list<Data>> HashArr(size);         //запись данных в односвязный список
    for (int i = 0; i < size; ++i)      
    {
        fl = HASH(arr[i].data, size);       //поиск индекса        
        if (!HashArr[fl].empty())
            ++colision;
        HashArr[fl].push_front(arr[i]);
    }
    return HashArr;
}

int HashSeach(string key, vector<forward_list<Data>> HashArr)     //поиск по хешированной таблице
{
    int size = HashArr.size();
    int hash_key = HASH(key, size);
    for (auto& i : HashArr[hash_key])
    {
        if (i.data == key)
        {
            return i.index;
...
int main()
{
    ...
    vector<forward_list<Data>> HashArr;
